...
CREATE TABLE IF NOT EXISTS `ums_user` (
  ...
  `status` tinyint(1) DEFAULT '1' COMMENT '账号启用状态: 0->禁用; 1->启用',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='C端用户表(账号密码体系)';
...
CREATE TABLE IF NOT EXISTS `pms_spu` (
  ...
  `detail_html` text COMMENT '商品富文本详情(PC/H5展示)',
...
CREATE TABLE IF NOT EXISTS `pms_sku` (
  ...
  `spu_id` bigint(20) NOT NULL COMMENT '关联SPU_ID',
  ...
  `image` varchar(255) DEFAULT NULL COMMENT '规格图片(如黑色手机图)',
  `attributes` json DEFAULT NULL COMMENT '规格属性JSON(如 {"color":"black", "storage":"256G"})',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `oms_order` (
  ...
  `order_sn` varchar(64) NOT NULL COMMENT '订单编号(系统生成，展示给用户)',
...
CREATE TABLE IF NOT EXISTS `oms_order_item` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `order_id` bigint(20) NOT NULL COMMENT '关联订单主表ID',
  `order_sn` varchar(64) NOT NULL COMMENT '订单编号(冗余字段，便于查询)',
  `spu_id` bigint(20) NOT NULL COMMENT '商品SPU_ID',
  `sku_id` bigint(20) NOT NULL COMMENT '商品SKU_ID',
  `sku_name` varchar(255) NOT NULL COMMENT '购买时SKU名称(快照)',
  `sku_pic` varchar(255) DEFAULT NULL COMMENT '购买时SKU图片(快照)',
  `sku_price` decimal(10,2) NOT NULL COMMENT '购买时价格(快照)',
